...
const urnify = (objectId) =>
  Buffer.from(objectId).toString("base64").replace(/=/g, "");
...
async function listModels() {
  ...
  let response = await new ObjectsApi().getObjects(
    BUCKET,
    { limit: 64 },
    null,
    token
  ...
  while (response.body.next) {
    ...
    response = await new ObjectsApi().getObjects(
      BUCKET,
      { limit: 64, startAt },
      null,
      token
  ...
  try {
    ...
    if (err.statusCode === 404) {
      await new BucketsApi().createBucket(
        { bucketKey: BUCKET, policyKey: "temporary" },
        {},
        null,
        token
      );
...
async function uploadModel(objectName, buffer, rootFilename) {
  const token = await getAccessToken(INTERNAL_TOKEN_SCOPES);
  const response = await new ObjectsApi().uploadObject(
    BUCKET,
    objectName,
    buffer.byteLength,
    buffer,
...
async function getMetaDtaGuid(urn) {
  const token = await getAccessToken(INTERNAL_TOKEN_SCOPES);
  const metaData = await new DerivativesApi().getMetadata(urn, {}, null, token);
  const guid = metaData.body.data.metadata[0].guid;
  const response = await new DerivativesApi().getModelviewProperties(
    urn,
    guid,
    {
      forceget: true,
